      ...
      <v-col cols="12">
        <v-card>
          <v-card-title class="page-title">
            <strong>ขอเปลี่ยนข้อมูล</strong>
          </v-card-title>
          <div class="pa-md-6 pa-3">
            <RequestEditForm v-if="initialData" @cancel="$router.push('/request')" @submit="confirmSubmit" />
          </div>
        </v-card>
      </v-col>
    </v-row>
...
<script>
import RequestEditForm from '~/components/RequestEditForm.vue'
export default {
  ...
  methods: {
    ...
    async fetchInitialData () { if (!this.setUserToken()) { await this.$router.replace('/login'); return } this.$Notiflix.loading(); try { const response = await this.$axios.$get('/requests/edit/form'); this.initialData = response.result } catch (error) { this.$Notiflix.remove(); await this.showError(error, 'ไม่สามารถดึงข้อมูลสำหรับยื่นคำร้องได้'); await this.$router.replace('/request'); return } this.$Notiflix.remove() },
    async confirmSubmit (form) { const confirm = await this.$swal({ icon: 'question', title: 'ยืนยันการบันทึกคำร้อง', text: 'กรุณาตรวจสอบข้อมูลก่อนยืนยัน', showCancelButton: true, confirmButtonText: 'ยืนยัน', cancelButtonText: 'ยกเลิก', confirmButtonColor: '#4fb24d' }); if (!confirm.isConfirmed) { return } this.$Notiflix.loading(); try { const data = new FormData(); data.append('data', JSON.stringify({ changeTypes: form.changeTypes })); Object.entries(form.evidences).forEach(([key, evidence]) => { if (evidence.status && evidence.file) { data.append(`evidences[${key}]`, evidence.file) } }); await this.$axios.$post('/requests/edit', data); this.$Notiflix.remove(); await this.$swal({ icon: 'success', title: 'บันทึกคำร้องเรียบร้อยแล้ว', confirmButtonText: 'ตกลง', confirmButtonColor: '#4fb24d' }); await this.$router.push('/request') } catch (error) { this.$Notiflix.remove(); await this.showError(error, 'ไม่สามารถบันทึกคำร้องได้') } },
