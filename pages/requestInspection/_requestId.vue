      ...
      <v-col cols="12">
        <v-card v-if="request">
          <v-card-title class="request-header">
            ...
            <strong>รายละเอียดคำร้องหนังสือรับรองการตรวจสอบใบอนุญาต</strong>
          </v-card-title>
          <div class="pa-md-6 pa-3">
            ...
            <DetailSection title="ข้อมูลสถานที่ทำงาน">
              <div class="detail-text">
                <strong>ประเภท:</strong> {{ workplaceTypeText }}
              </div><div class="detail-text mt-2">
                <strong>สถานที่:</strong> {{ request.details && request.details.workplaceName || '-' }}
              </div>
            </DetailSection>
            <v-divider class="my-5" />
            <DetailSection title="ด้านวิชาชีพที่ขอตรวจสอบ">
            ...
            <DetailSection title="สรุปค่าธรรมเนียม">
              <div v-for="item in feeItems" :key="item.code" class="d-flex justify-space-between fee-row">
                <span>{{ item.name }}</span><span>{{ formatPrice(item.amount) }} บาท</span>
...
const evidenceLabels = { workplace_certificate: 'หนังสือรับรองจากหน่วยงาน', id_card: 'สำเนาบัตรประจำตัวประชาชนหรือเอกสารประจำตัว', inspection_letter: 'หนังสือรับรองขอตรวจสอบใบอนุญาต', payment_receipt: 'สำเนาใบเสร็จรับเงินค่าธรรมเนียม', other_evidence: 'เอกสารหลักฐานอื่น ๆ' }
export default {
  name: 'RequestInspectionDetailPage',
  components: { DetailSection },
  data () { return { request: null } },
  computed: {
    backPath () { return localStorage.getItem('accessTokenUser') ? '/request' : '/requestInspection' },
    ...
    workplaceTypeText () { return { government: 'หน่วยงานส่วนราชการ', private: 'หน่วยงานภาคเอกชน', other: 'อื่น ๆ' }[this.request.details?.workplaceType] || '-' },
    selectedProfessions () { return Object.entries(this.request.details?.professions || {}).filter(([, value]) => value).map(([key]) => ({ key, label: professionLabels[key] || key })) },
  ...
  methods: { async fetchRequest () { const token = localStorage.getItem('accessTokenUser'); let requestCall; if (token) { this.$axios.setToken(token, 'Bearer'); requestCall = () => this.$axios.$get(`/requests/inspection/${this.$route.params.requestId}`) } else { this.$axios.setToken(false); const identity = this.getPublicIdentity(); if (!identity) { await this.$router.replace('/requestInspection'); return } requestCall = () => this.$axios.$post(`/inspection/${this.$route.params.requestId}/detail`, identity) } this.$Notiflix.loading(); try { const response = await requestCall(); this.request = response.result } catch (error) { this.$Notiflix.remove(); await this.$swal({ icon: 'error', title: 'ไม่สามารถดึงรายละเอียดคำร้องได้', text: error.response?.data?.message || 'ไม่พบคำร้องหรือท่านไม่มีสิทธิ์ดูข้อมูลนี้', confirmButtonText: 'ปิด', confirmButtonColor: '#327531' }); await this.$router.replace(this.backPath); return } this.$Notiflix.remove() }, getPublicIdentity () { try { const data = JSON.parse(sessionStorage.getItem('inspectionApplicant') || '{}'); return data.CustomerID && data.mobile ? { CustomerID: data.CustomerID, mobile: data.mobile } : null } catch { return null } }, formatPrice (value) { return Number(value || 0).toLocaleString('th-TH', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) } }
}
</script>
<style scoped>.request-header { background-color: #3d8f6a; color: #fff; font-size: 26px; font-weight: bold; }.summary-label { color: #424242; font-size: 20px; font-weight: bold; }.summary-value { color: #327531; font-size: 20px; font-weight: bold; }.detail-row { display: flex; align-items: flex-start; padding: 8px 0; font-size: 22px; }.detail-text, .fee-row { font-size: 21px; }.fee-row { padding: 5px 0; }.total-price { color: #327531; font-size: 24px; }</style>
