      ...
      <v-col cols="12">
        <v-card v-if="request">
          <v-card-title class="request-header">
            ...
            <strong>รายละเอียดคำร้องขอใบแทนใบอนุญาต</strong>
          </v-card-title>
          <div class="pa-md-6 pa-3">
            ...
            <DetailSection title="ด้านวิชาชีพที่ขอใบแทน">
              <div v-for="item in licenseItems" :key="item.professionType" class="detail-row">
                <v-icon color="#4fb24d" class="mr-2">
                  mdi-check-circle
                </v-icon>
                <div><strong>{{ professionLabel(item.professionType) }}</strong><div>เลขใบอนุญาต {{ item.licenseNumber || '-' }}</div></div>
              </div>
            </DetailSection>
            <v-divider class="my-5" />
            <DetailSection title="สาเหตุการขอใบแทน">
            ...
            <DetailSection title="วิธีการรับเอกสาร">
              ...
              <div v-if="request.delivery && request.delivery.method === 'ems'" class="detail-text mt-2">
                {{ documentAddressText }}
              </div>
            </DetailSection>
            <v-divider class="my-5" />
...
const evidenceLabels = { id_card: 'สำเนาบัตรประจำตัวประชาชนหรือเอกสารประจำตัว', residence: 'สำเนาทะเบียนบ้านหรือหลักฐานแสดงถิ่นที่อยู่', photo: 'ภาพถ่ายขนาด 1 นิ้ว จำนวน 2 ภาพ' }

export default {
  name: 'RequestReplacementDetailPage',
  ...
  computed: {
    ...
    reasonText () { return { damaged: 'ชำรุด', lost: 'สูญหาย' }[this.request.details?.reason] || '-' },
    evidenceItems () {
      return Object.entries(this.request.details?.evidences || {}).filter(([, value]) => value.status).map(([key, value]) => ({
        key,
        label: key === 'license_evidence' ? this.licenseEvidenceLabel : (evidenceLabels[key] || key),
        url: value.file?.url || ''
      }))
    },
    licenseEvidenceLabel () {
      return this.request.details?.reason === 'damaged'
        ? 'สำเนาบัตรสมาชิกที่ถูกทำลาย หรือหนังสือแจ้งความกรณีสูญหาย'
        : 'สำเนาหนังสือสำคัญการเป็นสมาชิก/สำเนาใบอนุญาตที่ถูกทำลาย หรือหนังสือแจ้งความกรณีสูญหาย'
    },
  ...
  methods: {
    async fetchRequest () {
      ...
      try { const response = await this.$axios.$get(`/requests/replacement/${this.$route.params.requestId}`); this.request = response.result } catch (error) { this.$Notiflix.remove(); await this.$swal({ icon: 'error', title: 'ไม่สามารถดึงรายละเอียดคำร้องได้', text: error.response?.data?.message || 'ไม่พบคำร้องหรือท่านไม่มีสิทธิ์ดูข้อมูลนี้', confirmButtonText: 'ปิด', confirmButtonColor: '#327531' }); await this.$router.replace('/request'); return }
...
@media screen and (max-width: 600px) { .request-header { font-size: 22px; } .detail-row, .detail-text, .fee-row { font-size: 19px; } }
</style>
