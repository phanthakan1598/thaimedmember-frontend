      ...
      <v-col cols="12">
        <v-card v-if="request">
          <v-card-title class="request-header">
            ...
            </v-btn><strong>รายละเอียดคำร้องต่ออายุบัตรสมาชิก / ขอบัตรสมาชิก</strong>
          </v-card-title>
          <div class="pa-md-6 pa-3">
            ...
            <DetailSection title="ข้อมูลการขอบัตรสมาชิก">
              <div class="detail-text">
                <strong>เลขสมาชิก:</strong> {{ request.details && request.details.memberNumber || '-' }}
              </div><div class="detail-text mt-2">
                <strong>เหตุผล:</strong> {{ reasonText }}
              </div><div v-if="request.details && request.details.lostDate" class="detail-text mt-2">
                <strong>วันที่สูญหาย:</strong> {{ request.details.lostDate }}
              </div>
            </DetailSection>
            <v-divider class="my-5" />
...
const evidenceLabels = { id_card: 'สำเนาเอกสารประจำตัว', residence: 'สำเนาทะเบียนบ้าน', professional_license: 'สำเนาใบอนุญาตประกอบวิชาชีพ', photo: 'ภาพถ่ายขนาด 1 นิ้ว จำนวน 2 ภาพ', member_card_original: 'บัตรสมาชิกตัวจริง', police_report: 'ใบแจ้งความกรณีบัตรสมาชิกสูญหาย', expired_member_card: 'บัตรสมาชิกเดิมที่หมดอายุ', other_evidence: 'เอกสารหลักฐานอื่น ๆ' }
export default {
  name: 'RequestRenewDetailPage',
  ...
  computed: {
    ...
    reasonText () { return { new_card: 'ทำบัตรใหม่', expired: 'บัตรเดิมหมดอายุ', damaged: 'ชำรุด', lost: 'สูญหาย' }[this.request.details?.reason] || '-' },
  ...
  methods: { async fetchRequest () { const token = localStorage.getItem('accessTokenUser'); if (!token) { await this.$router.replace('/login'); return } this.$axios.setToken(token, 'Bearer'); this.$Notiflix.loading(); try { const response = await this.$axios.$get(`/requests/renew/${this.$route.params.requestId}`); this.request = response.result } catch (error) { this.$Notiflix.remove(); await this.$swal({ icon: 'error', title: 'ไม่สามารถดึงรายละเอียดคำร้องได้', text: error.response?.data?.message || 'ไม่พบคำร้องหรือท่านไม่มีสิทธิ์ดูข้อมูลนี้', confirmButtonText: 'ปิด', confirmButtonColor: '#327531' }); await this.$router.replace('/request'); return } this.$Notiflix.remove() }, formatPrice (value) { return Number(value || 0).toLocaleString('th-TH', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) } }
}
</script>
